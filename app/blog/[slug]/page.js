    ...
    <main className="min-h-screen py-10 px-1 flex items-center justify-center bg-white text-black">
      <div className="max-w-6xl w-full rounded-xl shadow-lg border flex flex-col md:flex-row overflow-hidden bg-white border-gray-200">
        ...
        <div className="md:w-1/2 w-full p-4 sm:p-6 md:p-10 flex flex-col">
          <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold mb-3 sm:mb-4 text-black">
            {blog.title}
          </h1>
          ...
          {Array.isArray(blog.tags) && blog.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4 sm:mb-6">
              {blog.tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-block px-2 py-1 rounded text-xs font-medium border bg-blue-100 text-blue-700 border-blue-300"
                >
                  #{tag}
                </span>
              ))}
          ...
          <article
            className="prose max-w-none prose-headings:text-blue-700 prose-a:text-blue-600 prose-img:rounded prose-pre:bg-gray-100 prose-sm sm:prose-base md:prose-lg text-black"
